...
CFLAGS          = -O2 -std=gnu11 -fno-builtin -nostdlib -nostdinc -Wall -march=rv64imafd_zifencei_zicsr -mcmodel=medany -ggdb3 -falign-functions=4
...
KERNEL_CFLAGS   = $(CFLAGS) -DS_CORE $(KERNEL_INCLUDE) -Wl,--defsym=TEXT_START=$(KERNEL_ENTRYPOINT) -T riscv.lds

USER_INCLUDE    = -I$(DIR_TINYLIBC)/include
USER_CFLAGS     = $(CFLAGS) -DS_CORE $(USER_INCLUDE)
...
run:
	@echo "[hint] At U-Boot prompt, type: loadboot"
	$(QEMU) $(QEMU_OPTS)

RUN_TIMEOUT ?= 20
run-timeout:
	@echo "[hint] Auto stop after $(RUN_TIMEOUT)s. At U-Boot prompt, type: loadboot"
	timeout $(RUN_TIMEOUT)s $(QEMU) $(QEMU_OPTS)

run-same:
	@echo "[hint] Auto input loadboot and stop after $(RUN_TIMEOUT)s"
	(sleep 3; echo 'loadboot') | timeout $(RUN_TIMEOUT)s $(QEMU) $(QEMU_OPTS) || true

stop:
	pkill -9 -f '[q]emu-system-riscv64' || true
run-smp:
	$(QEMU) $(QEMU_OPTS) 
...
.PHONY: all dirs clean floppy asm gdb run run-timeout run-same stop run-smp debug debug-smp minicom viewlog run-net debug-net
...
$(DIR_BUILD)/%: $(DIR_TEST_PROJ)/%.c $(OBJ_CRT0) $(LIB_TINYC) riscv.lds FORCE
...
$(ELF_CREATEIMAGE): dirs $(SRC_CREATEIMAGE)
