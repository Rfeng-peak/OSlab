...
# Default to task3 demo set: two scheduler outputs + two lock tasks + sleep + timer + airplane.
SRC_USER    ?= $(DIR_TEST_PROJ)/print1.c $(DIR_TEST_PROJ)/print2.c $(DIR_TEST_PROJ)/lock1.c $(DIR_TEST_PROJ)/lock2.c $(DIR_TEST_PROJ)/sleep.c $(DIR_TEST_PROJ)/timer.c $(DIR_TEST_PROJ)/fly.c
...
.PHONY: all dirs clean floppy asm gdb run run-timeout run-same stop debug minicom
