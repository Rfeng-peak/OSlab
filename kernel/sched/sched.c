...
pid_t do_exec(long id_or_ptr, int argc, uint64_t arg0, uint64_t arg1, uint64_t arg2)
{
    ...
    /* construct default argv on user stack for argument-dependent programs */
    {
        regs_context_t *ur = (regs_context_t *)(pcb[idx].kernel_sp + sizeof(switchto_context_t));
        uintptr_t usp = ur->regs[2];
        usp = ROUNDDOWN(usp - 128, 16);
        char *stk = (char *)usp;
        uintptr_t *argv_ptrs = (uintptr_t *)(stk + 64);
        /* For rw (id==3): pass writable addresses converted to hex strings.
         * For others: pass print_location as argv[1]. */
        if (id == 3) {
            /* provide 2 writable addresses on stack */
            char *buf1 = stk, *buf2 = stk + 24;
            ptr_t addr1 = usp + 128, addr2 = usp + 136;
            /* format as hex strings */
            buf1[0]='0'; buf1[1]='x';
            int v = (int)(addr1 >> 28) & 0xF; buf1[2]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr1 >> 24) & 0xF; buf1[3]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr1 >> 20) & 0xF; buf1[4]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr1 >> 16) & 0xF; buf1[5]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr1 >> 12) & 0xF; buf1[6]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr1 >> 8) & 0xF;  buf1[7]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr1 >> 4) & 0xF;  buf1[8]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr1) & 0xF;       buf1[9]=(v<10)?(v+'0'):(v-10+'a');
            buf1[10] = 0;
            buf2[0]='0'; buf2[1]='x';
            v = (int)(addr2 >> 28) & 0xF; buf2[2]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr2 >> 24) & 0xF; buf2[3]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr2 >> 20) & 0xF; buf2[4]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr2 >> 16) & 0xF; buf2[5]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr2 >> 12) & 0xF; buf2[6]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr2 >> 8) & 0xF;  buf2[7]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr2 >> 4) & 0xF;  buf2[8]=(v<10)?(v+'0'):(v-10+'a');
            v = (int)(addr2) & 0xF;       buf2[9]=(v<10)?(v+'0'):(v-10+'a');
            buf2[10] = 0;
            argv_ptrs[0] = (uintptr_t)buf1;  /* argv[0] = addr1 hex */
            argv_ptrs[1] = (uintptr_t)buf2;  /* argv[1] = addr2 hex */
            argv_ptrs[2] = 0;
            ur->regs[10] = 2;  /* argc = 2 (program name + 1 arg) */
        } else {
            stk[0]='p'; stk[1]=0; stk[2]='2'; stk[3]=0;
            argv_ptrs[0] = usp;
            argv_ptrs[1] = usp + 2;
            argv_ptrs[2] = 0;
            ur->regs[10] = 2;
