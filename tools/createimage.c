...
static void create_image(int nfiles, char *files[])
{
    ...
    /* populate task info from filenames */
    for (int i = 0; i < tasknum && i < TASK_MAXNUM; i++) {
        const char *src = files[i + 2];
        char *dst = taskinfo[i].name;
        int j;
        for (j = 0; j < TASK_NAME_LEN - 1 && src[j]; j++)
            dst[j] = src[j];
        dst[j] = '\0';
        taskinfo[i].id = i;
    }
    ...
    for (int fidx = 0; fidx < nfiles; ++fidx) {
        ...
        if (strcmp(*files, "bootblock") == 0) {
            ...
            write_padding(img, &phyaddr, start_phyaddr + TASK_SIZE);
        }
...
static void write_img_info(int nbytes_kernel, task_info_t *taskinfo,
                           short tasknum, FILE * img)
{
    uint16_t kernel_secs = (uint16_t)NBYTES2SEC(TASK_SIZE);

    (void)nbytes_kernel;

    /*
     * Write task names into boot sector at TASK_INFO_OFFSET.
     * Layout: 1 byte task_count + tasknum * TASK_NAME_LEN bytes of names.
     */
    fseek(img, TASK_INFO_OFFSET, SEEK_SET);
    uint8_t count = (uint8_t)tasknum;
    fwrite(&count, 1, 1, img);
    for (int i = 0; i < tasknum; i++) {
        fwrite(taskinfo[i].name, TASK_NAME_LEN, 1, img);
    }

    /*
     * [p1-task1/2] store kernel size in sectors at 0x1fc,
     * and keep the 0x55aa signature at the end of the boot sector.
     */
    fseek(img, OS_SIZE_LOC, SEEK_SET);
    fwrite(&kernel_secs, sizeof(kernel_secs), 1, img);
